...
import com.github.youopensource.redhat.devtools.intellij.telemetry.core.ITelemetryService;
import com.github.youopensource.redhat.devtools.intellij.telemetry.core.util.AnonymizeUtils;
import com.github.youopensource.redhat.devtools.intellij.telemetry.core.util.TimeUtils;
...
public class TelemetryMessageBuilder {
    ...
    static class StartupMessage extends Message<StartupMessage> {
        ...
        private StartupMessage(ServiceFacade service) {
            super(TelemetryEvent.Type.STARTUP, "startup", service);
        }
    }
    ...
    static class ShutdownMessage extends Message<ShutdownMessage> {
        ...
        private ShutdownMessage(ServiceFacade service) {
            this(TimeUtils.toLocalTime(ApplicationManager.getApplication().getStartTime()), service);
        }
        ...
        ShutdownMessage(LocalDateTime startup, LocalDateTime shutdown, ServiceFacade service) {
            super(TelemetryEvent.Type.SHUTDOWN, "shutdown", service);
    ...
    public static class ActionMessage extends Message<ActionMessage> {
        ...
        private ActionMessage(String name, ServiceFacade service) {
            super(TelemetryEvent.Type.ACTION, name, service);
            started();
        }
    ...
    private abstract static class Message<T extends Message<?>> {
        ...
        private final TelemetryEvent.Type type;
        private final Map<String, String> properties = new HashMap<>();
        private final String name;
        private final ServiceFacade service;

        private Message(TelemetryEvent.Type type, String name, ServiceFacade service) {
            this.name = name;
            this.type = type;
